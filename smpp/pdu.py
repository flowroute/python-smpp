...
# Inserting certain referenced dicts in here means
# they can be declared in the same order as in the spec.
maps = {}
...
mandatory_parameter_lists = {
    # SMPP v3.4, section 4.1.1, table 4-1, page 46
    'bind_transmitter': [
    ...
    # SMPP v3.4, section 4.1.2, table 4-2, page 47
    'bind_transmitter_resp': [
        {'name': 'system_id',               'min': 1, 'max': 16,  'var': True,              'type': 'string',        'map': None}
    ],

    # SMPP v3.4, section 4.1.3, table 4-3, page 48
    'bind_receiver': [
        {'name': 'system_id',               'min': 1, 'max': 16,  'var': True,              'type': 'string',        'map': None},
        {'name': 'password',                'min': 1, 'max': 9,   'var': True,              'type': 'string',        'map': None},
        {'name': 'system_type',             'min': 1, 'max': 13,  'var': True,              'type': 'string',        'map': None},
    ...
    # SMPP v3.4, section 4.1.4, table 4-4, page 50
    'bind_receiver_resp': [
        {'name': 'system_id',               'min': 1, 'max': 16,  'var': True,              'type': 'string',        'map': None}
    ],

    # SMPP v3.4, section 4.1.5, table 4-5, page 51
    'bind_transceiver': [
        {'name': 'system_id',               'min': 1, 'max': 16,  'var': True,              'type': 'string',        'map': None},
        {'name': 'password',                'min': 1, 'max': 9,   'var': True,              'type': 'string',        'map': None},
        {'name': 'system_type',             'min': 1, 'max': 13,  'var': True,              'type': 'string',        'map': None},
    ...
    # SMPP v3.4, section 4.1.7.1, page 54
    'outbind': [
        {'name': 'system_id',               'min': 1, 'max': 16,  'var': True,              'type': 'string',        'map': None},
        {'name': 'password',                'min': 1, 'max': 9,   'var': True,              'type': 'string',        'map': None}
    ],

    # SMPP v3.4, section 4.2.1, table 4-7, page 56
    'unbind': [],

    # SMPP v3.4, section 4.2.2, table 4-8, page 56
    'unbind_resp': [],

    # SMPP v3.4, section 4.3.1, table 4-9, page 57
    'generic_nack': [],

    # SMPP v3.4, section 4.4.1, table 4-10, page 59-61
    'submit_sm': [
...
def encode_param_type(param, type_, min_=0, max_=None, map_=None):
    if param is None:
        hex_ = None
    elif map_ is not None:
        if type_ == 'integer' and isinstance(param, int):
            hex_ = ('%0' + str(min_*2) + 'x') % param
        else:
            hex_ = map_.get(param, ('%0' + str(min_*2) + 'x') % 0)
    elif type_ == 'integer':
        hex_ = ('%0' + str(min_*2) + 'x') % int(param)
    elif type_ == 'string':
        hex_ = param.encode('hex') + '00'
    elif type_ == 'xstring':
        hex_ = param.encode('hex')
    elif type_ == 'bitmask':
        hex_ = param
    elif type_ == 'hex':
        hex_ = param
    ...
    if hex_:
        if len(hex_) % 2:
            ...
            hex_ = '0' + hex_
    if None not in (max_, hex_) and len(hex_) > 2 * max_:
        raise ValueError("Value exceeds maximum size of {}.".format(max_))
    return hex_
